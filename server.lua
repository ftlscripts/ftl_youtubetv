...
RegisterServerEvent('ftl_tv:add')
AddEventHandler('ftl_tv:add', function(url, object, coords, scale, offset)
    ...
    TriggerClientEvent('ftl_tv:update', -1, tosend)
end)

RegisterServerEvent('ftl_tv:fetch')
AddEventHandler('ftl_tv:fetch', function()
    local tosend = {}
    ...
    TriggerClientEvent('ftl_tv:update', source, tosend)
end)

RegisterServerEvent('ftl_tv:setvolume')
AddEventHandler('ftl_tv:setvolume', function(id, volume)
    if volume >= 0 and volume <= 10 then
        if data[id] then

            data[id]['Volume'] = volume

            TriggerClientEvent('ftl_tv:updatevolume', -1, id, volume)

        end
    end
end)

RegisterServerEvent('ftl_tv:destroy')
AddEventHandler('ftl_tv:destroy', function(id)
    if data[id] then
        data[id] = nil
        TriggerClientEvent('ftl_tv:delete', -1, id)
    end
end)